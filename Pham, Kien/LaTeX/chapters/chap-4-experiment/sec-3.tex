...
\begin{figure}[htbp]
\centering
\resizebox{0.55\textwidth}{!}{\input{figures/tikz/circuit-DTLS.tex}}
\caption[Deep-level transient spectroscopy circuit]{Circuit used for the deep-level transient spectroscopy (DLTS)-type measurements on the gated thin-film structure. The external excitation is applied through the resistive divider formed by $R_1=\SI{1}{\mega\ohm}$ and $R_2=\SI{100}{\kilo\ohm}$. The voltage across $R_2$ is monitored so that the transient response of the device can be followed after each bias perturbation.}
