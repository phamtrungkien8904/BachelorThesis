...
# ============================================================
def calculate_n2d_and_mu(data, devcap, vth, area=DevArea, charge=Charge):
    vg4 = data[:, 0]
    i34 = data[:, 2]
    v12 = -data[:, 6]
    ...
    # result in cm^-2 because of division by 1e4
    n2d = (devcap * np.abs(vg4_eff)) / (area * charge * 1e4)

    mu = sigma2d / (charge * n2d)

    return vg4, vg4_eff, n2d, mu, sigma2d
...
# ============================================================
VG4_1, VG4eff_1, n2d_1, mob_1, sigma2d_1 = calculate_n2d_and_mu(Data1, C_PaN,  Vth_PaN)
VG4_2, VG4eff_2, n2d_2, mob_2, sigma2d_2 = calculate_n2d_and_mu(Data2, C_PS,   Vth_PS)
VG4_3, VG4eff_3, n2d_3, mob_3, sigma2d_3 = calculate_n2d_and_mu(Data3, C_Trip, Vth_Trip)
...
print(f"PaN: VG4 = {vg_pan_m50:.2f} V, VG4-Vth = {VG4eff_1[np.argmin(np.abs(VG4_1 + 50))]:.2f} V, n2D = {n2d_pan_m50:.3e} cm^-2, mu = {mu_pan_m50:.3e} cm^2/Vs")
print(f"PS: VG4 = {vg_ps_m50:.2f} V, VG4-Vth = {VG4eff_2[np.argmin(np.abs(VG4_2 + 50))]:.2f} V, n2D = {n2d_ps_m50:.3e} cm^-2, mu = {mu_ps_m50:.3e} cm^2/Vs")
print(f"Trip_C12: VG4 = {vg_trip_m50:.2f} V, VG4-Vth = {VG4eff_3[np.argmin(np.abs(VG4_3 + 50))]:.2f} V, n2D = {n2d_trip_m50:.3e} cm^-2, mu = {mu_trip_m50:.3e} cm^2/Vs")
