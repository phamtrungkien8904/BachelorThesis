...
for label, voltage, data, color, marker in dataset:
    ...
    plt.plot(t*1e3, -I_charge*1e6, label=label + f": C = {-Q_charge/voltage*1e9:.2f} nF", color=color, ls='-', lw = 2, markevery=400, markeredgecolor='white', markeredgewidth=1, markersize=7, marker=marker)

plt.axhline(y=0.0, color='black', ls='--', lw=1)
plt.xlabel('Time (ms)', fontsize=18)
plt.ylabel(r'Current ($\mu$A)', fontsize=18)
plt.title('Current vs. Time (Charge process)', fontsize=20)
plt.xlim(0, 50)
plt.ylim(0, 10)
