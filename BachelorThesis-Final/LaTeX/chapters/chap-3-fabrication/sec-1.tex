...
\begin{figure}[htbp]
    ...
    \begin{minipage}{0.50\linewidth}
        \centering
        \resizebox{\linewidth}{!}{\input{figures/tikz/oFET-3D.tex}}
    \end{minipage}
    \caption[Layer and contact schematic of the OFET structure]{Schematic representation of the assembled OFET structure. The left panel shows the top-view contact layout, while the right panel shows the corresponding 3D layer structure. The central green region denotes the molecular semiconductor, the red diagonal paths indicate the four injection contacts, the orange pads provide external electrical access, and the side pads marked G connect to the bottom gate. Adapted from Ref.~\cite{Kersting2026FiguresCollection}.}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{|p{1.8cm}|p{3.2cm}|p{5cm}|}
        ...
        \hline
    \end{tabular}
    \caption[Layer sequence of the fabricated device]{Layer sequence of the fabricated device, including material thicknesses and lithography masks. Adapted from Ref.~\cite{Kersting2026FiguresCollection}.}
