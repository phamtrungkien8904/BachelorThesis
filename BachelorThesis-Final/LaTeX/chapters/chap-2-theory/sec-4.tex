...
film rather than by its external shape.

\begin{figure}[htbp]
    \centering
    \scalebox{1.2}{\input{figures/tikz/classical-vdP.tex}}
...
solution of the van der Pauw equation
\begin{equation}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{figures/gvdP-simulation.pdf}
    \caption[Potential and carrier-density distributions in a gated van der
    Pauw structure]{Numerically calculated potential and hole-density
    distributions in a gated van der Pauw structure. A finite measurement
    current produces a spatially varying sheet potential. Since the local
    gate-induced charge depends on the voltage difference between the gate and
    the semiconductor, the carrier density is spatially varying as well. The
    two non-injecting probe potentials define the representative channel
    potential $V_\mathrm{C}$ used in the charge-density correction. The
    hole-density map shows pronounced accumulation of holes near the
    current-injecting contacts, where the imposed current enters and leaves the
    organic film. This local enhancement can result from the electrical
    boundary conditions and does not, by itself, demonstrate the presence of a
    Schottky barrier.}
    \label{fig:gvdP_simulation}
\end{figure}

Figure~\ref{fig:gvdP_simulation} illustrates why the internal potential
