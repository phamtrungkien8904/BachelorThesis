...
channel. In gated organic thin-film structures, however, the contact
resistance can be large and strongly gate dependent. It can therefore change the slope of a
...
Figure~\ref{fig:vdp_sim_results}. The calculation is used only to visualize how
fixed contact potentials and insulating boundaries can produce a nonuniform
in-plane potential and a corresponding nonuniform hole-density map. It is not a
quantitative simulation of the complete gated device, because the vertical gate
stack is not solved explicitly. The numerical procedure is summarized in
Appendix~\ref{app:vdp_simulation}.
...
\begin{figure}[htbp]
    ...
    calculated from the Boltzmann approximation and shows hole
...
two-terminal field-effect transfer curve, in which the measured voltage
includes both channel and contact contributions. 
...
the transport that governs actual gated thin-film operation.
